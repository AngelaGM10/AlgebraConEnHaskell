...
Finalmente hemos implementado uno de los ideales canónicos, el ideal cero, \texttt{<0>}. A continuación, damos la definición de ideal principal.\\
...
Mediante la función \texttt{from Id}, definida a continuación, mostramos la lista de los generadores de \texttt{(Id xs)}. 
\index{\texttt{fromId}}
\begin{code}
...
A continuación veremos una función cuyo objetivo es comprobar que el resultado de una operación \texttt{op} sobre dos ideales calcula el ideal correcto. Para ello, la operación debería proporcionar un “testigo” de forma que el ideal calculado tenga los mismos elementos. Es decir, si $z_k$ es un elemento del conjunto de generadores de \texttt{(Id zs)}, $z_k$ tiene una expresión como combinación lineal de \texttt{xs} e \texttt{ys}, cuyos coeficientes vienen dados por \texttt{as} y \texttt{bs}, respectivamente. 
